...
ACTIVITY
    ...
    *start [issue-id] [--issue-tracker] [--time-tracker] [--manual]*
      Starts a new activity. Use --manual option if you don't need to use an
      Issue Tracker. If during the configuration operation, the Issue
      ...
      will be used to extract the activity name from the issue and to change
      the its status. Via the *workflow* tool, it's possible to create a
      rule for updating issue status and done ratio on activity start,
      automatically assigning it to current user, and optionally add a
      comment.

    *stop*
      Use this command when you've to stop working on your activity. Via the
      *workflow* tool, it's possible to create a rule for updating issue
      status and done ratio on activity stop and optionally add a comment.

    *resume*
      If you have previously stopped your activity, you can always resume it
